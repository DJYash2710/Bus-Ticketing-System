...
import { cancelBookingInTx, logBookingCancellationAudits, } from "./cancelBookingCore.js";
import { AuditAction, AuditEntityType } from "../../core/audit/actions.js";
import { auditLogFrom } from "../../core/audit/auditLog.service.js";
export async function createBooking(input, audit) {
    ...
    const taxAmount = Math.round(baseAmount * env.gstRate * 100) / 100;
    ...
    const booking = await prisma.$transaction(async (tx) => {
        ...
        if (updatedSeats.count !== seats.length) {
            throw new ApiError(409, "Some seats were just held by another user. Please try again.");
        }
...
export async function cancelBooking(bookingId, userId, audit) {
    const booking = await prisma.booking.findFirst({
        where: {
            id: bookingId,
            userId,
