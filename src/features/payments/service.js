...
function toStripeInitiateResponse(payment, clientSecret, paymentState) {
...
function isStripePiRefreshable(stripeStatus, mappedStatus) {
    if (stripeStatus === "canceled") {
        return true;
    }
    return mappedStatus === "failed" || mappedStatus === "cancelled";
}
async function finalizeStripePaymentIfSucceeded(payment, providerRef, rawResponse, audit) {
...
async function handleInFlightStripePaymentIntent(payment, stripeStatus, providerRef, rawResponse, audit) {
    if (stripeStatus === "succeeded") {
        ...
        if (isLatePaymentAfterHoldExpiry(paymentWithBooking)) {
            ...
            return toStripeInitiateResponse(refunded ?? payment, null, refunded?.status === PaymentStatus.REFUNDED ? "completed" : "processing");
        }
        try {
            const finalized = await finalizeStripePaymentIfSucceeded(payment, providerRef, rawResponse, audit);
...
async function reuseOrRefreshStripePayment(payment, audit) {
    if (payment.status === PaymentStatus.SUCCESS) {
    ...
    if (payment.providerRef) {
        ...
        if (STRIPE_PI_IN_FLIGHT.has(remote.stripeStatus)) {
            return handleInFlightStripePaymentIntent(payment, remote.stripeStatus, remote.providerRef, remote.rawResponse, audit);
        }
        if (STRIPE_PI_PAYABLE.has(remote.stripeStatus)) {
            const clientSecret = await stripePaymentProvider.getClientSecret(payment.providerRef);
    ...
    return toStripeInitiateResponse(refreshed.payment, refreshed.clientSecret, "ready");
}
function logPaymentCreated(payment, bookingId, audit, userId) {
...
async function attachStripePaymentIntent(payment, bookingId) {
...
async function initiateMockPayment(booking, userId, audit) {
...
async function initiateStripePayment(booking, userId, audit) {
    if (!isStripeConfigured()) {
        throw new ApiError(500, "STRIPE_SECRET_KEY is not configured");
    }
    if (!env.stripe.publishableKey) {
        throw new ApiError(500, "STRIPE_PUBLISHABLE_KEY is not configured (required for mobile checkout)");
    }
    const existing = await prisma.payment.findUnique({ where: { bookingId: booking.id } });
    if (existing) {
        ...
        if (existing.provider === PAYMENT_PROVIDERS.MOCK && existing.status === PaymentStatus.PENDING) {
            ...
            const { payment: upgraded, clientSecret } = await attachStripePaymentIntent(existing, booking.id);
        ...
        if (existing.status === PaymentStatus.FAILED || existing.status === PaymentStatus.CANCELLED) {
            ...
            const { payment: linked, clientSecret } = await attachStripePaymentIntent(reset, booking.id);
    ...
    const { payment: linked, clientSecret } = await attachStripePaymentIntent(payment, booking.id);
    logPaymentCreated(linked, booking.id, audit, userId);
    return toStripeInitiateResponse(linked, clientSecret, "ready");
}
...
export async function confirmPayment(paymentId, userId, audit) {
    ...
    if (payment.provider === PAYMENT_PROVIDERS.STRIPE) {
        throw new ApiError(400, "Stripe payments are completed via the payment gateway. Do not confirm manually.");
    }
    if (payment.status === PaymentStatus.SUCCESS) {
...
async function syncPendingStripePayment(payment) {
    if (env.paymentProvider !== PAYMENT_PROVIDERS.STRIPE ||
    ...
    if (remote.stripeStatus === "succeeded") {
        if (isLatePaymentAfterHoldExpiry(paymentWithBooking)) {
            ...
            try {
                await finalizeStripePaymentIfSucceeded(payment, remote.providerRef, remote.rawResponse);
            }
            catch (err) {
    ...
    return ((await prisma.payment.findUnique({ where: { id: payment.id } })) ?? payment);
}
