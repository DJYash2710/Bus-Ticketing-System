...
export type LatePaymentRefundResult = {
  ...
  outcome:
    | 'auto_refunded'
    | 'already_refunded'
    | 'refund_failed_manual_reconciliation';
  refundId?: string;
};
...
export function isLatePaymentAfterHoldExpiry(payment: PaymentWithBooking): boolean {
  if (
...
export function shouldAutoRefundAfterFinalizeError(
  err: unknown,
...
function isStripeAlreadyRefundedError(err: unknown): boolean {
...
export async function refundLatePaymentAfterHoldExpiry(
  input: LatePaymentRefundInput,
  ...
  if (!isLatePaymentAfterHoldExpiry(payment)) {
    throw new ApiError(
      400,
      'Automatic refund only applies to payments received after hold expiry',
    );
  }

  let refundId: string | undefined;
  let refundRawResponse: string | undefined;
...
async function markRefundPendingForReconciliation(
  payment: PaymentWithBooking,
  input: LatePaymentRefundInput,
  err: unknown,
): Promise<void> {
